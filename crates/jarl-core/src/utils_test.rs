...
/// Get fixed text using a pre-built package cache.
pub fn get_fixed_text_with_cache(text: Vec<&str>, rule: &str, cache: &Arc<PackageCache>) -> String {
    let mut output: String = String::new();

    for txt in text.iter() {
        let original_content = txt;
        let modified_content = apply_fixes(txt, rule, false, None, None, Some(cache));
